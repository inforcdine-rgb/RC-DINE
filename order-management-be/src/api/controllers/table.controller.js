...
const updateName = async (req, res) => {
    try {
        ...
        const payload = req.body;
        logger('debug', `Update table name request ${hotelId}-${tableId} ${JSON.stringify(payload)}`);

        const valid = tableNameValidation(payload);
        if (valid.error) {
            logger('error', `Update table name validation failed: ${valid.error.message}`);
            return res.status(STATUS_CODE.BAD_REQUEST).send({ message: valid.error.message });
        }

        const result = await tableService.updateName(hotelId, tableId, payload);
        return res.status(STATUS_CODE.OK).send(result);
    } catch (error) {

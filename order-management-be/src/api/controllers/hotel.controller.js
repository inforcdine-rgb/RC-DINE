...
const getPrinterSettings = async (req, res) => {
    try {
        const hotelId = await resolveHotelAccess(req.user, req.params.id);
        const result = await hotelService.getPrinterSettings(hotelId);
        return res.status(STATUS_CODE.OK).send(result);
    } catch (error) {
        logger('error', 'Error in getPrinterSettings controller', { error });
        return res.status(error.code || 500).send({ message: error.message });
    }
};
...
const updatePrinterSettings = async (req, res) => {
    try {
        const hotelId = await resolveHotelAccess(req.user, req.params.id);
        const validation = printerSettingsValidation(req.body);
        if (validation.error) {
            return res.status(STATUS_CODE.BAD_REQUEST).send({ message: validation.error.message });
        }
        const result = await hotelService.updatePrinterSettings(hotelId, validation.value);
        return res.status(STATUS_CODE.OK).send(result);
    } catch (error) {
        logger('error', 'Error in updatePrinterSettings controller', { error });
        return res.status(error.code || 500).send({ message: error.message });
    }
};
...
const uploadLogo = async (req, res) => {
    try {
        ...
        logger('error', 'Error uploading hotel logo', {
...
const removeLogo = async (req, res) => {
    try {
        const hotelId = await resolveHotelAccess(req.user, req.params.id);
        const result = await hotelService.removeLogo(hotelId);
        return res.status(STATUS_CODE.OK).send(result);
    } catch (error) {
        logger('error', 'Error removing hotel logo', { error });
        return res.status(error.code || 500).send({ message: error.message });
    }
};

...
const placeOrder = async (req, res) => {
    try {
        ...
        if (result?.order?.orderId) {
            emitToOrder(result.order.orderId, 'order-status-updated', {
                orderId: result.order.orderId,
                orderNumber: result.order.orderNumber,
                status: 'PENDING',
                order: result.order,
...
const createWalkInOrder = async (req, res) => {
    try {
        ...
        logger('info', 'Manager POS live order event emitted', {
            hotelId,
...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        emitToOrder(orderId, 'order-status-updated', livePayload);

        emitToHotel(hotelId, 'order-status-updated', livePayload);

        logger('info', 'Live order status event emitted', livePayload);
...
const cancelOrder = async (req, res) => {
    try {
        ...
        emitToHotel(result.hotelId, 'order-cancelled', livePayload);
        emitToOrder(orderId, 'order-cancelled', livePayload);
        emitToHotel(result.hotelId, 'order-status-updated', livePayload);
        emitToOrder(orderId, 'order-status-updated', livePayload);

        logger('info', `Order ${orderId} cancelled successfully`);
        logger('info', 'Order cancellation live event emitted', livePayload);

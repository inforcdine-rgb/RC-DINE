...
const remove = async (options) => {
    try {
        return await db.notifications.destroy(options);
    } catch (error) {
        const err = error?.errors ? error.errors[0]?.message : undefined;
        logger('error', `Error occurred while deleting notification: ${err || error.message}`);
        throw CustomError(error.code, err || error.message);
    }
};
...
const restore = async (options) => {
    try {
        return await db.notifications.restore(options);
    } catch (error) {
        const err = error?.errors ? error.errors[0]?.message : undefined;
        logger('error', `Error occurred while restoring notification: ${err || error.message}`);
        throw CustomError(error.code, err || error.message);
    }
};

export default { save, find, update, remove, restore };

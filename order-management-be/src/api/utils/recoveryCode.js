...
const RECOVERY_CODE_PATTERN = /^\d{4}$/;
...
const dummyRecoveryCodeHash = bcrypt.hash('RC0000', SALT_ROUNDS);
...
export const hashRecoveryCode = async (digits) => {
    ...
    if (!normalized) throw new Error('Recovery code must be exactly 4 numeric digits.');
    return bcrypt.hash(normalized, SALT_ROUNDS);
};

export const compareRecoveryCode = async (digits, storedHash) => {
    const normalized = normalizeRecoveryCode(digits) || 'RC0000';
...
export const safeDummyRecoveryCodeComparison = async (digits) =>
    bcrypt.compare(normalizeRecoveryCode(digits) || 'RC0000', await dummyRecoveryCodeHash);

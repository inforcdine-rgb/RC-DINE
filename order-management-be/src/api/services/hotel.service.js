...
const dashboard = async (hotelId, user) => {
    try {
        const hotelOptions = {
            where: { id: hotelId },
            attributes: ['id', 'name', 'logo', 'openTime', 'closeTime', 'address', 'careNumber']
        };
...
const getPaymentSettings = async (hotelId) => {
    try {
        ...
        const result = {
            ...
            gstPercent: Number(hotel.gstPercent ?? 0),
            discountEnabled: !!hotel.discountEnabled,
            discountType: hotel.discountType || 'PERCENT',
            discountValue: Number(hotel.discountValue ?? 0)
        };
...
const updatePaymentSettings = async (hotelId, payload) => {
    try {
        ...
        const updateData = {
            ...
            discountType: discountEnabled ? discountType || 'PERCENT' : null,
...
const getPrinterSettings = async (hotelId) => {
    try {
        ...
        return {
            printerWidth: hotel.printerWidth || '58',
            address: hotel.address || '',
            phone: hotel.careNumber || '',
            gstNumber: hotel.gstNumber || '',
            logo: hotel.logo || '',
            showLogo: hotel.receiptShowLogo !== false,
            footerMessage: hotel.receiptFooterMessage || 'Thank you! Visit again.'
        };
    } catch (error) {
        logger('error', 'Error in getPrinterSettings service', { error });
        throw CustomError(error.code || 500, error.message);
    }
};

const updatePrinterSettings = async (hotelId, payload) => {
    try {
        const hotel = await hotelRepo.find({ where: { id: hotelId } });
        if (!hotel) throw CustomError(STATUS_CODE.NOT_FOUND, 'Hotel not found');
...
const updateLogo = async (hotelId, logoUrl) => {
    const hotel = await hotelRepo.find({ where: { id: hotelId } });
    if (!hotel) throw CustomError(STATUS_CODE.NOT_FOUND, 'Hotel not found');
    if (hotel.logo && hotel.logo !== logoUrl) await deleteImage(hotel.logo);
    await hotelRepo.update({ id: hotelId }, { logo: logoUrl });
    return { message: 'Hotel logo updated successfully', logo: logoUrl };
};

const removeLogo = async (hotelId) => {
    const hotel = await hotelRepo.find({ where: { id: hotelId } });
    if (!hotel) throw CustomError(STATUS_CODE.NOT_FOUND, 'Hotel not found');
    if (hotel.logo) await deleteImage(hotel.logo);
    await hotelRepo.update({ id: hotelId }, { logo: null });
    return { message: 'Hotel logo removed successfully', logo: null };
};

...
function MenuCard({ data = {}, currentOrder = {}, name = '', tableNumber, handleClick = () => {}, handleOnChange = () => {}, tipAmount = 0, onTipAmountChange = () => {} }) {
    ...
    const { categories, itemsByCategoryName } = useMemo(() => {
        ...
        pagesList.forEach((page) => {
            ...
            if (page.type === types.category && Array.isArray(page.data)) {
                page.data.forEach((cat) => { if (cat?.id && cat?.name) extractedCategories.push({ id: cat.id, name: cat.name }); });
            }
    ...
    const cartItems = useMemo(() => {
        ...
        return Object.keys(local).filter((id) => id !== 'lastUpdated' && Number(local[id]?.quantity) > 0).map((id) => ({
            id, name: local[id]?.menuName || local[id]?.name, price: local[id]?.price, quantity: Number(local[id]?.quantity)
        }));
    }, [currentOrder]);

    const cartTotal = useMemo(() => cartItems.reduce((sum, item) => sum + (Number(item.price) || 0) * item.quantity, 0), [cartItems]);
    const sgst = useMemo(() => Math.round(cartTotal * 0.025), [cartTotal]);
    const cgst = useMemo(() => Math.round(cartTotal * 0.025), [cartTotal]);
    const normalizedTipAmount = useMemo(() => (!isTipEnabled ? 0 : Math.max(0, Number(tipAmount) || 0)), [isTipEnabled, tipAmount]);
    const finalAmount = useMemo(() => (!cartItems.length ? 0 : cartTotal + sgst + cgst + normalizedTipAmount), [cartItems.length, cartTotal, cgst, sgst, normalizedTipAmount]);
    ...
    const visibleCategories = useMemo(() => {
        ...
        return categories.map((category) => {
            ...
            const catMatch = String(category.name || '').toLowerCase().includes(query);
            const filteredItems = catMatch ? items : items.filter((item) => String(item.name || '').toLowerCase().includes(query));
        ...
        <>
            <div className="d-flex justify-content-center w-100 h-100">
                <Card
                    ...
                    style={{ backgroundImage: `linear-gradient(135deg, rgba(73,172,96,0.95), rgba(8,24,45,0.92)), url(${MenuBackgroundImg})` }}
                >
                    <Card.Body className="customer-order-body">
                        <header className="customer-order-header">
                            ...
                            <div className="customer-order-table">Table {tableNumber ?? '-'}<span className="customer-order-table-dot" /></div>
                        ...
                        <div className="customer-order-content">
                            {categories.length > 0 && (
                                <div className="customer-search-wrap">
                                    <input className="customer-search-input" type="search" value={searchText}
                                        placeholder="Search food items..." onChange={(e) => setSearchText(e.target.value)} />
                                </div>
                            )}
                                            ...
                                            {category.items.map((item) => {
                                                ...
                                                return (
                                                    <div key={item.id} className="customer-food-card">
                                                        ...
                                                        {/* ── UPDATED: dish photo or initial fallback ── */}
                                                        <div className="customer-food-media">
                                                            {item.image ? (
                                                                <img
                                                                    src={item.image}
                                                                    alt={item.name}
                                                            ...
                                                            <div
                                                                className="customer-food-initial"
                                                        ...
                                                        <div className="customer-food-action">
                                                            {quantity > 0 ? (
                                                                <div className="qty-control" role="group" aria-label={`Quantity for ${item.name}`}>
                                                                    <Button className="qty-btn" onClick={() => setMenuQuantity(item, Math.max(0, quantity - 1))} aria-label={`Decrease ${item.name} quantity`}>-</Button>
                                                                    <div className="qty-value" aria-live="polite">{quantity}</div>
                                                                    <Button className="qty-btn" onClick={() => setMenuQuantity(item, quantity + 1)} aria-label={`Increase ${item.name} quantity`}>+</Button>
                                                                </div>
                                                            ) : (
                                                                <Button className="customer-add-btn" onClick={() => setMenuQuantity(item, 1)} aria-label={`Add ${item.name}`}>Add</Button>
                                                            )}
                                                        </div>
                                                    </div>
                                                );
                        ...
                        <div className="cart-items">
                            {cartItems.map((item) => (
                                <div key={item.id} className="cart-item-row">
                                    ...
                                    <div className="cart-item-qty">{item.quantity} x ₹ {item.price}</div>
                                </div>
                            ))}
                            <div className="cart-tax-divider" style={{ borderTop: '1px solid rgba(255,255,255,0.1)', margin: '8px 0' }} />
                            <div className="cart-item-row" style={{ opacity: 0.8, fontSize: '0.9em' }}><div className="cart-item-name">Subtotal</div><div className="cart-item-qty">₹ {cartTotal}</div></div>
                            <div className="cart-item-row" style={{ opacity: 0.9, fontSize: '0.9em', alignItems: 'center' }}>
                                <label className="cart-item-name" style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                                    <input type="checkbox" checked={isTipEnabled} onChange={(e) => { const next = e.target.checked; setIsTipEnabled(next); if (!next) { setCustomTip(''); onTipAmountChange(0); } }} />
                                    Add Tip
                                </label>
                                ...
                                <div style={{ marginTop: '6px', marginBottom: '6px' }}>
                                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                        {[20, 50, 100].map((amount) => (
                                            <Button key={amount} variant={normalizedTipAmount === amount ? 'light' : 'outline-light'} size="sm" onClick={() => { setCustomTip(''); onTipAmountChange(amount); }}>₹{amount}</Button>
                                        ))}
                                        <input type="number" min="0" value={customTip} placeholder="Custom Amount" className="form-control form-control-sm" style={{ maxWidth: '150px' }}
                                            onChange={(e) => { const next = e.target.value; setCustomTip(next); onTipAmountChange(Math.max(0, Number(next) || 0)); }} />
                                    </div>
                                </div>
                            )}
                            <div className="cart-item-row" style={{ opacity: 0.8, fontSize: '0.9em' }}><div className="cart-item-name">CGST (2.5%)</div><div className="cart-item-qty">₹ {cgst}</div></div>
                            <div className="cart-item-row" style={{ opacity: 0.8, fontSize: '0.9em' }}><div className="cart-item-name">SGST (2.5%)</div><div className="cart-item-qty">₹ {sgst}</div></div>
                            <div className="cart-item-row" style={{ fontWeight: 'bold' }}><div className="cart-item-name">Final Amount</div><div className="cart-item-qty">₹ {finalAmount}</div></div>
                        </div>
                    ) : (
                        <div className="cart-empty">Add items to see your total.</div>
                    )}
                    <div className="cart-footer">
                        <div style={{ fontSize: '12px', opacity: 0.8, marginBottom: '8px' }}>No refund will be provided once payment is completed.</div>
                        <Button className="cart-proceed-btn" disabled={cartTotal <= 0 || isPlacing} onClick={handleProceedToPayment}>

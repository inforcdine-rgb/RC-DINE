...
import { playOrderPlacedSound } from '../../utils/sound';
import {
...
function* registerCustomerRequestSaga(action) {
    try {
        ...
        const result = yield service.registerCustomer(payload);
...
function* paymentConfirmationRequestSaga(action) {
    try {
        ...
        if (!payload.manual) {
            toast.info(`Payment confirmed. Thank you for choosing us! Your feedback means a lot.`);
...
function* verifyCustomerPaymentRequestSaga(action) {
    try {
        ...
        const activeOrder = {
            orderId: orderData.orderId || orderData.id || result?.orderId || result?.id,
        ...
        const trackingId = orderData.orderId || orderData.id || orderData.order?.id || result?.orderId || result?.id;
        const oldNotifications = JSON.parse(sessionStorage.getItem('rcdineNotifications') || '[]');
        const orderNotification = {
            id: `PLACED-${Date.now()}`,
            icon: '✔',
            title: 'Order Placed',
            text: 'Your order has been placed successfully. Tap to track your order.',
            status: 'PLACED',
            orderId: trackingId,
            read: false,
        ...
        toast.success('Payment successful! Order received! Opening tracking screen...');

        if (trackingId && typeof payload.navigate === 'function') {
            payload.navigate(`/cart/${trackingId}`);
        }
    } catch (error) {
